...
导出配色用 `--export-theme light`（默认白底）或 `--export-theme dark`（黑底白字）。导出选择独立于播放器主题，保留并调亮强调色。

导出默认沿用 CLI 播放器保存的字体；可用 `--export-font-chinese wenkai`、`--export-font-latin klee-one` 分别覆盖本次导出。选择、缺字回退及模块集成见[字体指南](./docs/font-selection.md)。

视频导出还支持 `--export-speed 1.5`，范围 0.25–3，默认原速；画面与旁白同步变速并保持音调。`--fps` 控制输出帧率，不是播放速度。
