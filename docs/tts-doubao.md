...
当前接入采用 Node 离线生成流程，支持豆包官方服务以及兼容豆包 V3 HTTP/SSE 协议的自部署／第三方服务。不在浏览器内实时请求 TTS，也不暴露可被外部滥用的生成端点。首页已按用户要求换为约 65 秒的水循环课，新旁白生成与对齐见[水循环说明](./water-cycle.md)。此前约 77 秒的二分查找配音、已对齐课程和旧版 87 秒资源均保留，归档页面位于 `/examples/binary-search`。没有发布到外网。
...
### 使用自部署 Qwen3-TTS 或第三方兼容接口

服务端需要兼容当前豆包 V3 HTTP/SSE 合同：接收相同的 POST JSON 与可选 `X-Api-Key`、`X-Api-Resource-Id`、`X-Api-Request-Id` 请求头，并以 `text/event-stream` 返回 `data: {"code":0,"data":"<base64音频>"}` 音频事件；可用 `code: 20000000` 表示结束。若返回字幕／词时间戳，还应放在兼容的 `sentence`、`subtitle` 或 `subtitles` 字段中。

另一台电脑上的 Qwen3-TTS 服务可这样配置（地址与音色按你的部署修改）：

```dotenv
DOUBAO_TTS_ENDPOINT=http://192.168.1.20:8000/api/v3/tts/unidirectional/sse
DOUBAO_TTS_SPEAKER=qwen3-tts-voice
DOUBAO_TTS_SPEECH_RATE=0
DOUBAO_SPEECH_API_KEY=
DOUBAO_TTS_RESOURCE_ID=
```

自定义地址只接受 `http://` 或 `https://`，因此支持局域网 HTTP 服务。接口地址中不能嵌入账号、密码或 URL 片段；需要鉴权时使用 `DOUBAO_SPEECH_API_KEY`，需要资源标识时使用 `DOUBAO_TTS_RESOURCE_ID`。两项留空时不会发送对应请求头。`DOUBAO_TTS_SPEAKER` 始终必填。

同一文稿在不同接口地址会使用不同缓存，不会误用豆包官方服务生成的音频。自定义服务的接口地址不会写进缓存清单，密钥也不会进入请求正文、缓存或浏览器资源。
...
接口回归测试使用本地模拟响应，覆盖官方默认地址、自定义兼容地址、可选鉴权头、SSE 跨包、空音频控制消息、音频拼接、错误与缓存，不消耗云端额度。
