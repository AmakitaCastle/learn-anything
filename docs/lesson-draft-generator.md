...
提示版本 `0.4.1` 默认使用全文板书：完整旁白逐句、随语音逐字书写，已讲过的全文保留。模型指定每段的 `visualId` 和局部 `emphasis`；每张图必须有对应完整讲解，纯文字课不要求额外的 `board.write`。新生成材料缺少全文模式、图文关联或图示动作会被拒绝。模型材料中的整句、跨句与重叠重点属于可选标记错误：生成模块仅删除违规标记，重新严格校验整份材料，并在 `generation.json` 的 `removedEmphasis` 中记录删除数量。人工导入仍会拒绝这些重点。
