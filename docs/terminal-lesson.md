...
若所有语音已合成，可把恢复命令中的 `--generate` 改为 `--cached`，确保缓存缺失时停止而不请求付费接口。编码器的微秒舍入及多段累加产生的浮点误差，在差异不超过一微秒时统一使用实测片段总长，避免最后一段全文板书或结束锚点被误判越界；其他时间范围仍严格校验。终端也会显示白名单中的音频处理与课程时间轴错误，不输出任意异常内容。
